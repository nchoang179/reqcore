...
/**
 * One row per served `/jobs.xml` request — the delivery receipt for job board
 * syndication.
 *
 * Feed distribution is pull-based, so without this table there is no record
 * anywhere that a board ever collected anything: not for the recruiter asking
 * "is my role actually out there?", and not for us when an aggregator quietly
 * stops fetching because the publisher account was suspended. A board going
 * silent is invisible until applications dry up, which is far too late.
 *
 * Untagged fetches (`board` null — a browser, a monitor, an unknown crawler)
 * are logged too, but the Promote tab reads only tagged ones. Each aggregator
 * is given its own `?board=` URL at submission time, so anything arriving
 * without one is not a board we can make a claim about.
 */
export const feedFetch = pgTable('feed_fetch', {
  id: text('id').primaryKey().$defaultFn(() => crypto.randomUUID()),
  /** `FEED_BOARDS` key from `?board=`, or null for an untagged request. */
  board: text('board'),
  /** How many jobs the response carried. Zero is the signal worth alerting on. */
  jobCount: integer('job_count').notNull(),
  userAgent: text('user_agent'),
  /**
   * Crawler infrastructure, not a person — this endpoint serves aggregators,
   * never candidates. It is the only way to tell two fetchers apart when one
   * of them forgets its `?board=` tag.
   */
  ipAddress: text('ip_address'),
  /** Time spent building the feed, to catch it degrading before a board times out. */
  durationMs: integer('duration_ms'),
  fetchedAt: timestamp('fetched_at').notNull().defaultNow(),
}, (t) => ([
  // Serves the only read this table has: newest fetch per board.
  index('feed_fetch_board_fetched_at_idx').on(t.board, t.fetchedAt.desc()),
  index('feed_fetch_fetched_at_idx').on(t.fetchedAt.desc()),
]))
