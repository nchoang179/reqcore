...
import {
  ...
  BarChart3, Sparkles, RefreshCw, AlertCircle, CheckCircle2, Rss,
    ...
    <section class="rounded-xl border border-surface-200 dark:border-surface-800 bg-white dark:bg-surface-950 overflow-hidden">
      <div class="border-b border-surface-100 dark:border-surface-800 px-5 py-4">
        ...
        <p class="mt-0.5 text-xs text-surface-500 dark:text-surface-400">
          Publishing already distributed this role. Nothing else to set up.
        </p>
      </div>
      ...
      <ul class="divide-y divide-surface-100 dark:divide-surface-800">
        ...
        <li class="px-5 py-3">
          <div class="flex items-start gap-3">
            <component
              :is="data.feed.eligible ? CheckCircle2 : AlertCircle"
              class="mt-0.5 size-4 shrink-0"
              :class="data.feed.eligible ? 'text-success-600 dark:text-success-400' : 'text-warning-600 dark:text-warning-400'"
            />
            <div class="min-w-0 flex-1">
              <p class="text-sm text-surface-700 dark:text-surface-300">
                {{ data.feed.eligible ? 'External job boards' : 'External job boards — not yet' }}
              </p>
              ...
              <p v-else class="mt-0.5 text-xs text-warning-700 dark:text-warning-400">
                {{ data.feed.reason }}
              </p>
              <NuxtLink
                v-if="!data.feed.eligible && data.feed.fixable"
